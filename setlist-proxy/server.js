...
app.use(cors({
  origin: function(origin, callback) {
    ...
    // Reject silently (false = no CORS headers, but no error thrown)
  ...
  try {
    ...
    const [moments, total] = await Promise.all([
      Moment.find({ user: userId, approvalStatus: 'approved', showInMoments: { $ne: false } })
        ...
        .skip(skip)
        .limit(limit)
        .select('title songName mediaType song performance thumbnailUrl mediaUrl externalVideoId createdAt venueName venueCity performanceDate')
        .lean(),
